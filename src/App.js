...
function getDefaultValue() {
    // функция проверяющая localStorage и если там что-то есть возвращает это значение, либо пустой массив
...
function App() {
    ...
    useEffect(() => {
        // при каждом обновлении books этот books записывается в localStorage
        localStorage.setItem("userBooksList", JSON.stringify(books));
    }, [books]);

    const addBook = (autor, nameBook) => {
    ...
    const findIndexForImg = (id) => {
        // индекс книги у которой изменяется картинка
        const index = books.findIndex((elem) => elem.id === id);
        setEditBookIndex(index);
    };

    const loadImg = (e) => {
        // загрузка картинки книге
        ...
        reader.addEventListener("load", () => {
            setBooks((books) => {
                ...
                const newBook = {
                    autor: books[editBookIndex].autor,
                    nameBook: books[editBookIndex].nameBook,
                    id: books[editBookIndex].id,
                    img: reader.result,
                };
        ...
        <>
            ...
            <div className="content">
                ...
                <BooksList
                    ...
                    findIndexForImg={findIndexForImg}
                    loadImg={loadImg}
                />
